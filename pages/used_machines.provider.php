...
if (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply')) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && 'form_save_error' !== $message) {
        header('Location: '. rex_url::currentBackendPage(array_merge($usedMachinesPageParams, ['entry_id' => $provider->provider_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
        header('Location: '. rex_url::currentBackendPage(array_merge($usedMachinesPageParams, ['message' => $message]), false));
    }
    exit;
}
// Delete
...
elseif ('changestatus' === $func) {
    ...
    header('Location: '. rex_url::currentBackendPage($usedMachinesPageParams));
    exit;
}
    ...
    <form action="<?= rex_url::currentBackendPage($usedMachinesPageParams) ?>" method="post">
		<div class="panel panel-edit">
			...
			<div class="panel-body">
				...
				<fieldset>
					<legend><?= rex_i18n::msg('d2u_machinery_export_basic_settings') ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
                            $provider = new Provider($entry_id);
                            $readonly = false;

                            BackendHelper::form_input('d2u_helper_name', 'form[name]', $provider->name, true, $readonly, 'text');
                            ...
                            BackendHelper::form_select('d2u_machinery_export_type', 'form[type]', $options, [$provider->type], 1, false, $readonly);

                            $options_lang = [];
                            foreach (rex_clang::getAll() as $rex_clang) {
                                $options_lang[$rex_clang->getId()] = $rex_clang->getName();
                            }
                            BackendHelper::form_select('d2u_machinery_export_clang', 'form[clang_id]', $options_lang, [$provider->clang_id]);
                            BackendHelper::form_input('d2u_machinery_export_company_name', 'form[company_name]', $provider->company_name, true, $readonly, 'text');
                            BackendHelper::form_input('d2u_machinery_export_company_email', 'form[company_email]', $provider->company_email, true, $readonly, 'email');
                            BackendHelper::form_input('d2u_machinery_export_customer_number', 'form[customer_number]', $provider->customer_number, false, $readonly, 'text');
                            $options_media = [];
                            ...
                            BackendHelper::form_select('d2u_machinery_export_media_manager_type', 'form[media_manager_type]', $options_media, [$provider->media_manager_type]);
                            BackendHelper::form_checkbox('d2u_helper_online_status', 'form[online_status]', 'online', 'online' === $provider->online_status, $readonly);
                        ?>
					</div>
				</fieldset>
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            BackendHelper::form_input('d2u_machinery_export_ftp_server', 'form[ftp_server]', $provider->ftp_server, false, $readonly, 'text');
                            BackendHelper::form_input('d2u_machinery_export_ftp_username', 'form[ftp_username]', $provider->ftp_username, false, $readonly, 'text');
                            BackendHelper::form_input('d2u_machinery_export_ftp_password', 'form[ftp_password]', $provider->ftp_password, false, $readonly, 'text');
                            BackendHelper::form_input('d2u_machinery_export_ftp_filename', 'form[ftp_filename]', $provider->ftp_filename, false, $readonly, 'text');
                        ?>
					</div>
				</fieldset>
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            BackendHelper::form_input('d2u_machinery_export_social_app_id', 'form[social_app_id]', $provider->social_app_id, false, $readonly, 'text');
                            BackendHelper::form_input('d2u_machinery_export_social_app_secret', 'form[social_app_secret]', $provider->social_app_secret, false, $readonly, 'text');
                        ?>
					</div>
				</fieldset>
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            BackendHelper::form_select('d2u_machinery_export_linkedin_type', 'form[linkedin_type]', $options, [$provider->linkedin_type], 1, false, $readonly);
                            BackendHelper::form_input('d2u_machinery_export_linkedin_id', 'form[linkedin_id]', $provider->linkedin_id, false, $readonly, 'text');
                            BackendHelper::form_infotext('d2u_machinery_export_linkedin_id_hint', 'linkedin_id_hint');
                        ?>
					</div>
				</fieldset>
			</div>
...
if ('' === $func) {
    ...
    $list->setColumnLabel('type', rex_i18n::msg('d2u_machinery_export_type'));
    $list->setColumnSortable('type');

    $list->removeColumn('online_status');
    $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . rex_url::currentBackendPage(array_merge($usedMachinesPageParams, ['func' => 'changestatus'])) . '&entry_id=###provider_id###"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
