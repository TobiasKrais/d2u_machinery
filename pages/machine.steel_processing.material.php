...
$steelProcessingPageParams = isset($steelProcessingPageParams) && is_array($steelProcessingPageParams) ? $steelProcessingPageParams : ['steel_processing_subpage' => 'material'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        ...
        if ('delete' === $material->translation_needs_update) {
            $material->delete(false);
        } elseif ($material->save()) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $material) {
        header('Location: '. rex_url::currentBackendPage(array_merge($steelProcessingPageParams, ['entry_id' => $material->material_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $material = new Material($material_id, (int) rex_config::get('d2u_helper', 'default_lang'));
    $material->material_id = $material_id; // Ensure correct ID in case language has no object

    // Check if object is used
    $referring_machines = $material->getReferringMachines();
...
?>
    <form action="<?= rex_url::currentBackendPage($steelProcessingPageParams) ?>" method="post">
		...
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_steel_material_class') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[material_id]" value="<?= $entry_id ?>">
				<fieldset>
					<legend><?= rex_i18n::msg('d2u_helper_data_all_lang') ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
                            // Do not use last object from translations, because you don't know if it exists in DB
                            $material = new Material($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
                            if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
                                $readonly = false;
                            }
                            BackendHelper::form_input('d2u_helper_name', 'form[internal_name]', $material->internal_name, true, $readonly, 'text');
                        ?>
					</div>
				</fieldset>
				<?php
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$material->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $material->name, $required, $readonly_lang, 'text');
                                ?>
							</div>
						</div>
					</fieldset>
...
if ('' === $func) {
    $query = 'SELECT materials.material_id, internal_name '
        ...
        . 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_steel_material_lang AS lang '
			. 'ON materials.material_id = lang.material_id AND lang.clang_id = '. (int) rex_config::get('d2u_helper', 'default_lang') .' ';
    ...
    $list->setColumnSortable('material_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###material_id###'] + $csrfToken->getUrlParams());
