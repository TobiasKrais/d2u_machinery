...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
	...
	foreach (rex_clang::getAll() as $rex_clang) {
		...
		if ('delete' === $industry_sector->translation_needs_update) {
			$industry_sector->delete(false);
		} elseif ($industry_sector->save()) {
	...
	if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $industry_sector) {
		header('Location: '. rex_url::currentBackendPage(['entry_id' => $industry_sector->industry_sector_id, 'func' => 'edit', 'message' => $message], false));
	} else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
	...
	if (0 === $industry_sector_id) {
		$form = rex_post('form', 'array', []);
		$industry_sector_id = $form['industry_sector_id'];
	}
	$industry_sector = new IndustrySector($industry_sector_id, (int) rex_config::get('d2u_helper', 'default_lang'));
	$industry_sector->industry_sector_id = $industry_sector_id;
	...
	$industry_sector = new IndustrySector($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
?>
	<form action="<?= BackendHelper::getCurrentBackendPage([], ['message', 'message_type']) ?>" method="post">
		...
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_industry_sectors') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[industry_sector_id]" value="<?= $entry_id ?>">
				<?php
				    foreach (rex_clang::getAll() as $rex_clang) {
...
if ('' === $func) {
	$query = 'SELECT industry_sectors.industry_sector_id, name, online_status '
		...
		. 'LEFT JOIN '. rex::getTablePrefix() .'d2u_machinery_industry_sectors_lang AS lang '
			. 'ON industry_sectors.industry_sector_id = lang.industry_sector_id AND lang.clang_id = '. (int) rex_config::get('d2u_helper', 'default_lang') .' ';
	...
	$list->setColumnSortable('industry_sector_id');
	...
	if (rex::getUser() instanceof rex_user && (rex::getUser()->isAdmin() || rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
		$list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . BackendHelper::getCurrentBackendPage(['func' => 'changestatus', 'entry_id' => '###industry_sector_id###'], [], true) . '"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
		$list->setColumnLayout(rex_i18n::msg('status_online'), ['', '<td class="rex-table-action">###VALUE###</td>']);
		$list->addColumn(rex_i18n::msg('delete_module'), '<i class="rex-icon rex-icon-delete"></i> ' . rex_i18n::msg('delete'));
		$list->setColumnLayout(rex_i18n::msg('delete_module'), ['', '<td class="rex-table-action">###VALUE###</td>']);
		$list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###industry_sector_id###'] + $csrfToken->getUrlParams());
