...
$agitatorPageParams = isset($agitatorPageParams) && is_array($agitatorPageParams) ? $agitatorPageParams : ['agitator_subpage' => 'agitator'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        if (false === $agitator) {
            ...
            $agitator->agitator_id = $agitator_id; // Ensure correct ID in case first language has no object
        ...
        if ('delete' === $agitator->translation_needs_update) {
            $agitator->delete(false);
        } elseif ($agitator->save()) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $agitator) {
        header('Location: '. rex_url::currentBackendPage(array_merge($agitatorPageParams, ['entry_id' => $agitator->agitator_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
        header('Location: '. rex_url::currentBackendPage(array_merge($agitatorPageParams, ['message' => $message]), false));
    }
    exit;
}
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $agitator = new Agitator($agitator_id, (int) rex_config::get('d2u_helper', 'default_lang'));
    ...
    if (0 === count($referring_agitator_types)) {
        $agitator->delete(true);
    } else {
        $message = '<ul>';
        foreach ($referring_agitator_types as $referring_agitator_type) {
            $message .= '<li><a href="index.php?page=d2u_machinery/machine/agitators&agitator_subpage=agitator_type&func=edit&entry_id='. $referring_agitator_type->agitator_type_id .'">'. $referring_agitator_type->name.'</a></li>';
        }
        $message .= '</ul>';
...
?>
    <form action="<?= rex_url::currentBackendPage($agitatorPageParams) ?>" method="post">
		<?= $csrfToken->getHiddenField() ?>
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_agitators') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[agitator_id]" value="<?= 'edit' === $func ? $entry_id : 0 ?>">
				<?php
                    foreach (rex_clang::getAll() as $rex_clang) {
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            $agitator = new Agitator($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
...
if ('' === $func) {
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###agitator_id###'] + $csrfToken->getUrlParams());
