...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
        ...
        if (false === $used_machine) {
            ...
            if (\rex_addon::get('d2u_videos')->isAvailable()) {
                ...
                foreach ($video_ids as $video_id) {
                    if ($video_id > 0) {
                        $used_machine->videos[$video_id] = new \TobiasKrais\D2UVideos\Video($video_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                    }
                }
            }
        } else {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $used_machine) {
        header('Location: '. rex_url::currentBackendPage(array_merge($usedMachinesPageParams, ['entry_id' => $used_machine->used_machine_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
...
if ('' === $func) {
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . rex_url::currentBackendPage(array_merge($usedMachinesPageParams, ['func' => 'changestatus'])) . '&entry_id=###used_machine_id###"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###used_machine_id###'] + $csrfToken->getUrlParams());
    ...
    $list->setColumnFormat(rex_i18n::msg('d2u_helper_open_frontend'), 'custom', static function ($params) {
        ...
        return BackendHelper::getFrontendLinkButton((new \TobiasKrais\D2UMachinery\UsedMachine((int) $listParams->getValue('used_machine_id'), (int) rex_config::get('d2u_helper', 'default_lang')))->getUrl());
    });
