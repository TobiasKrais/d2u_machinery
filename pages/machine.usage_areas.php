...
elseif ('priority_down' === $func || 'priority_up' === $func) {
    ...
    $usage_area->usage_area_id = $entry_id; // Ensure correct ID in case language has no object

    if ('priority_down' === $func) {
        ++$usage_area->priority;
        $usage_area->save();
    } elseif ($usage_area->priority > 1) {
        --$usage_area->priority;
        $usage_area->save();
    }
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$usage_area->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $usage_area->name, $required, $readonly_lang, 'text');
                                ?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            BackendHelper::form_input('header_priority', 'form[priority]', $usage_area->priority, true, $readonly, 'number');

                            $options = [];
                            foreach (Category::getAll((int) rex_config::get('d2u_helper', 'default_lang')) as $category) {
                                $options[$category->category_id] = $category->name;
                            }
                            BackendHelper::form_select('d2u_machinery_usage_areas_categories', 'form[category_ids][]', $options, $usage_area->category_ids, 10, true, $readonly);
                        ?>
					</div>
				</fieldset>
			</div>
...
if ('' === $func) {
    $query = 'SELECT usage_areas.usage_area_id, name, priority, '
        . '(SELECT MAX(priority) FROM '. \rex::getTablePrefix() .'d2u_machinery_usage_areas) AS max_priority '
    ...
    $list->setColumnSortable('usage_area_id');
    ...
    $list->setColumnFormat('priority', 'custom', static function ($params) {
        ...
        return BackendHelper::getPriorityButtons(
            (int) $listParams->getValue('usage_area_id'),
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###usage_area_id###'] + $csrfToken->getUrlParams());
