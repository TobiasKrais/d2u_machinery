...
// save settings
if (1 === (int) filter_input(INPUT_POST, 'btn_save', FILTER_VALIDATE_INT) || 1 === (int) filter_input(INPUT_POST, 'btn_apply')) {
        ...
        if (false === $category) {
            ...
            if (\rex_addon::get('d2u_videos')->isAvailable()) {
                ...
                foreach ($video_ids as $video_id) {
                    $category->videos[$video_id] = new Video($video_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                }
            }
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply') && $category instanceof Category) {
			...
			<div class="panel-body">
						...
						<div class="panel-body-wrapper slide">
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    ...
                                    d2u_addon_backend_helper::form_medialistfield('d2u_machinery_category_pdfs', (int) ('1'. $rex_clang->getId()), $category->pdfs, $readonly_lang);
                                ?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            $category = new Category($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
                            if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
                                $readonly = false;
                            }

                            $options = ['-1' => rex_i18n::msg('d2u_machinery_category_parent_none')];
                            $selected_values = [];
                            foreach (Category::getAll((int) rex_config::get('d2u_helper', 'default_lang')) as $parent_category) {
                ...
                ?>
					<fieldset>
						...
						<div class="panel-body-wrapper slide">
							<?php
                                d2u_addon_backend_helper::form_checkbox('d2u_machinery_category_show_agitators', 'form[show_agitators]', 'show', 'show' === $category->show_agitators);
                            ?>
						</div>
					</fieldset>
				<?php
