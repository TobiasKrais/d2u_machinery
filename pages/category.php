...
// save settings
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save', FILTER_VALIDATE_INT) || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
...
if ('' === $func) {
    $query = 'SELECT categories.category_id, lang.name AS categoryname, parents_lang.name AS parentname, priority, '
        . '(SELECT MAX(priority) FROM '. \rex::getTablePrefix() .'d2u_machinery_categories) AS max_priority '
    ...
    $list->setColumnFormat(rex_i18n::msg('d2u_helper_open_frontend'), 'custom', static function ($params) {
        ...
        return BackendHelper::getFrontendLinkButton((new \TobiasKrais\D2UMachinery\Category((int) $listParams->getValue('category_id'), (int) rex_config::get('d2u_helper', 'default_lang')))->getUrl());
    });
