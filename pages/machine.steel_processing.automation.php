...
elseif ('delete_unused' === $func) {
    $automations = Automation::getUnused();
    if (is_array($automations)) {
        foreach ($automations as $automation) {
            $automation->delete(true);
        }

        echo rex_view::success(count($automations) .' ' . rex_i18n::msg('d2u_machinery_steel_automation_delete_unused_success') . $message);
    }
...
if ('' === $func) {
    ...
    $list->setColumnSortable('automation_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###automation_id###'] + $csrfToken->getUrlParams());
    ...
    if (false !== Automation::getUnused()) {
        echo '<p><a href="'. rex_url::currentBackendPage(array_merge($steelProcessingPageParams, ['func' => 'delete_unused']), false) .'"><button class="btn btn-save">'. rex_i18n::msg('d2u_machinery_steel_automation_delete_unused') .'</button></a></p><br><br>';
    }
}