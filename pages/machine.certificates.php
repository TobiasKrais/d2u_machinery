...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        ...
        if ('delete' === $certificate->translation_needs_update) {
            $certificate->delete(false);
        } elseif ($certificate->save()) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $certificate) {
        header('Location: '. rex_url::currentBackendPage(['entry_id' => $certificate->certificate_id, 'func' => 'edit', 'message' => $message], false));
    } else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $certificate = new Certificate($certificate_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
?>
	<form action="<?= BackendHelper::getCurrentBackendPage([], ['message', 'message_type']) ?>" method="post">
		...
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_certificates') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[certificate_id]" value="<?= $entry_id ?>">
				<?php
                    foreach (rex_clang::getAll() as $rex_clang) {
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$certificate->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $certificate->name, $required, $readonly_lang, 'text');
                                    BackendHelper::form_textarea('d2u_helper_description', 'form[lang]['. $rex_clang->getId() .'][description]', $certificate->description, 10, false, $readonly_lang, true)
                                ?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            $certificate = new Certificate($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
                            if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
                                $readonly = false;
                            }

                            BackendHelper::form_mediafield('d2u_helper_picture', '1', $certificate->pic, $readonly);
                        ?>
					</div>
				</fieldset>
...
if ('' === $func) {
    $query = 'SELECT certificates.certificate_id, name '
        ...
        . 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_certificates_lang AS lang '
			. 'ON certificates.certificate_id = lang.certificate_id AND lang.clang_id = '. (int) rex_config::get('d2u_helper', 'default_lang') .' ';
    ...
    $list->setColumnSortable('certificate_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###certificate_id###'] + $csrfToken->getUrlParams());
