...
$steelProcessingPageParams = isset($steelProcessingPageParams) && is_array($steelProcessingPageParams) ? $steelProcessingPageParams : ['steel_processing_subpage' => 'process'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $process) {
        header('Location: '. rex_url::currentBackendPage(array_merge($steelProcessingPageParams, ['entry_id' => $process->process_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            BackendHelper::form_input('d2u_helper_name', 'form[internal_name]', $process->internal_name, true, $readonly, 'text');
                        ?>
					</div>
				</fieldset>
				<?php
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$process->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $process->name, $required, $readonly_lang, 'text');
                                ?>
							</div>
						</div>
					</fieldset>
...
if ('' === $func) {
    ...
    $list->setColumnSortable('process_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###process_id###'] + $csrfToken->getUrlParams());
