...
$equipmentPageParams = isset($equipmentPageParams) && is_array($equipmentPageParams) ? $equipmentPageParams : ['equipment_subpage' => 'equipment'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $equipment) {
        header('Location: '. rex_url::currentBackendPage(array_merge($equipmentPageParams, ['entry_id' => $equipment->equipment_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    header('Location: '. rex_url::currentBackendPage($equipmentPageParams));
    exit;
}
...
if ('' === $func) {
    ...
    $list->setColumnSortable('equipment_id');
    ...
    $list->setColumnSortable('article_number');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . rex_url::currentBackendPage(array_merge($equipmentPageParams, ['func' => 'changestatus'])) . '&entry_id=###equipment_id###"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###equipment_id###'] + $csrfToken->getUrlParams());
