...
$steelProcessingPageParams = isset($steelProcessingPageParams) && is_array($steelProcessingPageParams) ? $steelProcessingPageParams : ['steel_processing_subpage' => 'tool'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        ...
        if ('delete' === $tool->translation_needs_update) {
            $tool->delete(false);
        } elseif ($tool->save()) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $tool) {
        header('Location: '. rex_url::currentBackendPage(array_merge($steelProcessingPageParams, ['entry_id' => $tool->tool_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $tool = new Tool($tool_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
?>
    <form action="<?= rex_url::currentBackendPage($steelProcessingPageParams) ?>" method="post">
		...
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_steel_tools') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[tool_id]" value="<?= $entry_id ?>">
				<fieldset>
					<legend><?= rex_i18n::msg('d2u_helper_data_all_lang') ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
                            // Do not use last object from translations, because you don't know if it exists in DB
                            $tool = new Tool($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
                            if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
                                $readonly = false;
                            }
                            BackendHelper::form_input('d2u_helper_name', 'form[internal_name]', $tool->internal_name, true, $readonly, 'text');
                        ?>
					</div>
				</fieldset>
				<?php
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$tool->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $tool->name, $required, $readonly_lang, 'text');
                                ?>
							</div>
						</div>
					</fieldset>
...
if ('' === $func) {
    $query = 'SELECT tools.tool_id, internal_name '
        ...
        . 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_steel_tool_lang AS lang '
			. 'ON tools.tool_id = lang.tool_id AND lang.clang_id = '. (int) rex_config::get('d2u_helper', 'default_lang') .' ';
    ...
    $list->setColumnSortable('tool_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###tool_id###'] + $csrfToken->getUrlParams());
