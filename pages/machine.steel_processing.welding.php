...
$steelProcessingPageParams = isset($steelProcessingPageParams) && is_array($steelProcessingPageParams) ? $steelProcessingPageParams : ['steel_processing_subpage' => 'welding'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        ...
        if ('delete' === $welding->translation_needs_update) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $welding) {
        header('Location: '. rex_url::currentBackendPage(array_merge($steelProcessingPageParams, ['entry_id' => $welding->welding_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $welding = new Welding($welding_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
?>
    <form action="<?= rex_url::currentBackendPage($steelProcessingPageParams) ?>" method="post">
		...
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?= rex_i18n::msg('d2u_machinery_steel_welding') ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[welding_id]" value="<?= $entry_id ?>">
				<fieldset>
					<legend><?= rex_i18n::msg('d2u_helper_data_all_lang') ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
                            // Do not use last object from translations, because you don't know if it exists in DB
                            $welding = new Welding($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            $readonly = true;
...
if ('' === $func) {
    $query = 'SELECT weldings.welding_id, internal_name '
        ...
        . 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_steel_welding_lang AS lang '
			. 'ON weldings.welding_id = lang.welding_id AND lang.clang_id = '. (int) rex_config::get('d2u_helper', 'default_lang') .' ';
    ...
    $list->setColumnSortable('welding_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###welding_id###'] + $csrfToken->getUrlParams());
