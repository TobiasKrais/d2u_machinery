...
$equipmentPageParams = isset($equipmentPageParams) && is_array($equipmentPageParams) ? $equipmentPageParams : ['equipment_subpage' => 'equipment_group'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $equipment_group) {
        header('Location: '. rex_url::currentBackendPage(array_merge($equipmentPageParams, ['entry_id' => $equipment_group->group_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
        header('Location: '. rex_url::currentBackendPage(array_merge($equipmentPageParams, ['message' => $message]), false));
    }
    exit;
}
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    if (0 === count($referring_equipments)) {
        ...
        foreach ($referring_equipments as $referring_equipment) {
            $message .= '<li><a href="index.php?page=d2u_machinery/machine/equipment&equipment_subpage=equipment&func=edit&entry_id='. $referring_equipment->equipment_id .'">'. $referring_equipment->name.'</a></li>';
        }
        $message .= '</ul>';
...
?>
    <form action="<?= rex_url::currentBackendPage($equipmentPageParams) ?>" method="post">
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$equipment_group->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $equipment_group->name, $required, $readonly_lang, 'text');
                                    BackendHelper::form_textarea('d2u_helper_description', 'form[lang]['. $rex_clang->getId() .'][description]', $equipment_group->description, 10, false, $readonly_lang, true);
                                ?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            BackendHelper::form_input('header_priority', 'form[priority]', $equipment_group->priority, true, $readonly, 'number');
                            BackendHelper::form_mediafield('d2u_helper_picture', '1', $equipment_group->picture, $readonly);
                        ?>
					</div>
				</fieldset>
			</div>
...
if ('' === $func) {
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###group_id###'] + $csrfToken->getUrlParams());
