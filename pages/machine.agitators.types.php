...
$agitatorPageParams = isset($agitatorPageParams) && is_array($agitatorPageParams) ? $agitatorPageParams : ['agitator_subpage' => 'agitator_type'];
...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    if (1 === (int) filter_input(INPUT_POST, 'btn_apply', FILTER_VALIDATE_INT) && false !== $agitator_type) {
        header('Location: '. rex_url::currentBackendPage(array_merge($agitatorPageParams, ['entry_id' => $agitator_type->agitator_type_id, 'func' => 'edit', 'message' => $message]), false));
    } else {
						...
						<div class="panel-body-wrapper slide">
							<?php
                                if ($rex_clang->getId() !== (int) rex_config::get('d2u_helper', 'default_lang')) {
                                    ...
                                    BackendHelper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$agitator_type->translation_needs_update], 1, false, $readonly_lang);
                                } else {
							...
							<div id="details_clang_<?= $rex_clang->getId() ?>">
								<?php
                                    BackendHelper::form_input('d2u_helper_name', 'form[lang]['. $rex_clang->getId() .'][name]', $agitator_type->name, $required, $readonly_lang, 'text');
                                ?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            BackendHelper::form_mediafield('d2u_helper_picture', '1', $agitator_type->pic, $readonly);
                            ...
                            BackendHelper::form_select('d2u_machinery_agitators', 'form[agitator_ids][]', $options_agitators, $agitator_type->agitator_ids, 10, true, $readonly);
                        ?>
					</div>
				</fieldset>
...
if ('' === $func) {
    ...
    $list->setColumnSortable('agitator_type_id');
    ...
    if (\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
        ...
        $list->setColumnParams(rex_i18n::msg('delete_module'), ['func' => 'delete', 'entry_id' => '###agitator_type_id###'] + $csrfToken->getUrlParams());
