...
class ProductionLine implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.production_line_id FROM '. \rex::getTablePrefix() .'d2u_machinery_production_lines AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_production_lines_lang AS default_lang '
                        .'ON main.production_line_id = default_lang.production_line_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('production_line_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
            ...
            if ($object->production_line_id > 0) {
                $objects[] = $object;
            }
        }
