 ...
 * Machine Options.
 */
class Option implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.option_id FROM '. \rex::getTablePrefix() .'d2u_machinery_options AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_options_lang AS default_lang '
                        .'ON main.option_id = default_lang.option_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('option_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
    ...
    {
        if ($this->option_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->option_id, $sourceClangId);
        if ($source->option_id <= 0 || ('' === $source->name && '' === $source->description)) {
            return false;
        }
