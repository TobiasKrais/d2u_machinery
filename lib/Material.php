 ...
 * Material.
 */
class Material implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public static function getAll($clang_id)
    {
        ...
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $materials[] = new self((int) $result->getValue('material_id'), $clang_id);
            $result->next();
        }
        return $materials;
    }
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $objects[] = new self((int) $result->getValue('material_id'), $clang_id);
            $result->next();
        }
    ...
    {
        if ($this->material_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->material_id, $sourceClangId);
        if ($source->material_id <= 0 || '' === $source->name) {
            return false;
        }
