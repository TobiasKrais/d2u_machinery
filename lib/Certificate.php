 ...
 * Certificates.
 */
class Certificate implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.certificate_id FROM '. \rex::getTablePrefix() .'d2u_machinery_certificates AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_certificates_lang AS default_lang '
                        .'ON main.certificate_id = default_lang.certificate_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('certificate_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
    ...
    {
        if ($this->certificate_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->certificate_id, $sourceClangId);
        if ($source->certificate_id <= 0 || ('' === $source->name && '' === $source->description)) {
            return false;
        }
