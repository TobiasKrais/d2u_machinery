 ...
 * Service option.
 */
class ServiceOption implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.service_option_id FROM '. \rex::getTablePrefix() .'d2u_machinery_service_options AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_service_options_lang AS default_lang '
                        .'ON main.service_option_id = default_lang.service_option_id AND default_lang.clang_id = :default_lang '
    ...
    {
        if ($this->service_option_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->service_option_id, $sourceClangId);
        if ($source->service_option_id <= 0 || ('' === $source->name && '' === $source->description)) {
            return false;
        }
