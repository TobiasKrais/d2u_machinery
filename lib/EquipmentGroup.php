 ...
 */
class EquipmentGroup implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.group_id FROM '. \rex::getTablePrefix() .'d2u_machinery_equipment_groups AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_equipment_groups_lang AS default_lang '
                        .'ON main.group_id = default_lang.group_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('group_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
    ...
    {
        if ($this->group_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->group_id, $sourceClangId);
        if ($source->group_id <= 0 || ('' === $source->name && '' === $source->description)) {
            return false;
        }
