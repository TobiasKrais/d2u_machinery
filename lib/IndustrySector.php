...
class IndustrySector implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.industry_sector_id FROM '. \rex::getTablePrefix() .'d2u_machinery_industry_sectors AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_industry_sectors_lang AS default_lang '
                        .'ON main.industry_sector_id = default_lang.industry_sector_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('industry_sector_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
            ...
            if ($object->industry_sector_id > 0) {
                $objects[] = $object;
            }
        }
