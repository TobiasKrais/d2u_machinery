 ...
 * Welding.
 */
class Welding implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    {
        if ($this->welding_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->welding_id, $sourceClangId);
        if ($source->welding_id <= 0 || '' === $source->name) {
            return false;
        }
    ...
    public function save()
    {
        ...
        if (0 === $this->welding_id || $pre_save_welding !== $this) {
            ...
            if (0 === $this->welding_id) {
                ...
                $query = 'UPDATE '. $query .' WHERE welding_id = '. (int) $this->welding_id;
            }
