 ...
 * Usage Area.
 */
class UsageArea implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.usage_area_id FROM '. \rex::getTablePrefix() .'d2u_machinery_usage_areas AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_usage_areas_lang AS default_lang '
                        .'ON main.usage_area_id = default_lang.usage_area_id AND default_lang.clang_id = :default_lang '
    ...
    {
        if ($this->usage_area_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->usage_area_id, $sourceClangId);
        if ($source->usage_area_id <= 0 || '' === $source->name) {
            return false;
        }
