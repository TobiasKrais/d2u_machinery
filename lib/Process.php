 ...
 * Process.
 */
class Process implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public function __construct($process_id, $clang_id)
    {
        ...
        if ($num_rows > 0) {
            $this->process_id = (int) $result->getValue('process_id');
    ...
    public static function getAll($clang_id)
    {
        ...
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $process[] = new self((int) $result->getValue('process_id'), $clang_id);
            $result->next();
        }
        return $process;
    }
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $objects[] = new self((int) $result->getValue('process_id'), $clang_id);
            $result->next();
        }
    ...
    {
        if ($this->process_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->process_id, $sourceClangId);
        if ($source->process_id <= 0 || '' === $source->name) {
            return false;
        }
    ...
    public function save()
    {
        ...
        $pre_save_process = new self($this->process_id, $this->clang_id);

        // saving the rest
        if (0 === $this->process_id || $pre_save_process !== $this) {
            $query = \rex::getTablePrefix() .'d2u_machinery_steel_process SET '
                    .'internal_name = :internal_name ';
            if (0 === $this->process_id) {
                $query = 'INSERT INTO '. $query;
            } else {
                $query = 'UPDATE '. $query .' WHERE process_id = '. (int) $this->process_id;
            }

            $result = \rex_sql::factory();
            $result->setQuery($query, [':internal_name' => $this->internal_name]);
            if (0 === $this->process_id) {
                $this->process_id = (int) $result->getLastId();
