 ...
 * Machine Feature.
 */
class Feature implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('feature_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
    ...
    {
        if ($this->feature_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->feature_id, $sourceClangId);
        if ($source->feature_id <= 0 || ('' === $source->name && '' === $source->description)) {
            return false;
        }
    ...
    public function save()
    {
        ...
        if (0 === $this->feature_id || $pre_save_feature !== $this) {
            ...
            if (0 === $this->feature_id) {
                ...
                $query = 'UPDATE '. $query .' WHERE feature_id = '. (int) $this->feature_id;
            }
