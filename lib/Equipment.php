...
class Equipment implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    ...
    public static function getTranslationHelperObjects($clang_id, $type)
    {
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.equipment_id FROM '. \rex::getTablePrefix() .'d2u_machinery_equipments AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_equipments_lang AS default_lang '
                        .'ON main.equipment_id = default_lang.equipment_id AND default_lang.clang_id = :default_lang '
        ...
        $objects = [];
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $id = (int) $result->getValue('equipment_id');
            $result->next();
            if ($id <= 0) {
                continue;
            }
    ...
    public function save()
    {
        ...
        if (0 === $this->equipment_id || $pre_save_object !== $this) {
            $query = \rex::getTablePrefix() .'d2u_machinery_equipments SET '
                    ...
                    .'group_id = '. ($this->group instanceof EquipmentGroup ? (int) $this->group->group_id : 0) .', '
                    .'online_status = :online_status ';

            if (0 === $this->equipment_id) {
                $query = 'INSERT INTO '. $query;
            } else {
                $query = 'UPDATE '. $query .' WHERE equipment_id = '. (int) $this->equipment_id;
            }
