 ...
 * Profile.
 */
class Profile implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
    /** @var int Database ID */
    ...
    {
        if ($this->profile_id <= 0 || $sourceClangId === $this->clang_id) {
            return false;
        }

        $source = new self($this->profile_id, $sourceClangId);
        if ($source->profile_id <= 0 || '' === $source->name) {
            return false;
        }
    ...
    public function save()
    {
        ...
        if (0 === $this->profile_id || $pre_save_profile !== $this) {
            ...
            if (0 === $this->profile_id) {
                ...
                $query = 'UPDATE '. $query .' WHERE profile_id = '. (int) $this->profile_id;
            }
