...
use TobiasKrais\D2UHelper\FrontendHelper as D2UHelperFrontendHelper;
...
class Category implements \TobiasKrais\D2UHelper\ITranslationHelper, \TobiasKrais\D2UHelper\ITranslateable
{
     ...
     * @see http://www.agriaffaires.com/html/rubtableauliste_compact_de.html Full List
     */
    ...
    public function __construct($category_id, $clang_id)
    {
        ...
        if (Extension::isActive('used_machines')) {
            ...
            if ((int) $d2u_machinery->getConfig('used_machine_article_id_rent') === (int) $d2u_machinery->getConfig('used_machine_article_id_sale')
                    && ($current_article_id === (int) $d2u_machinery->getConfig('used_machine_article_id_rent') || $current_article_id === (int) $d2u_machinery->getConfig('used_machine_article_id_sale'))) {
                if ('rent' === filter_input(INPUT_GET, 'offer_type') || 'sale' === filter_input(INPUT_GET, 'offer_type')) {
                    $this->offer_type = filter_input(INPUT_GET, 'offer_type');
                } else {
                    // Get offer type from used machine
                    $used_category_id = 0;
                    if (\rex_addon::get('url')->isAvailable()) {
                        $url_namespace = D2UHelperFrontendHelper::getUrlNamespace();
                        if ('used_rent_category_id' === $url_namespace || 'used_sale_category_id' === $url_namespace) {
                            $used_category_id = D2UHelperFrontendHelper::getUrlId();
                        }
            ...
            } elseif ($current_article_id === $d2u_machinery->getConfig('used_machine_article_id_rent')) {
    ...
    public static function getAll($clang_id)
    {
        ...
        if ('priority' === \rex_addon::get('d2u_machinery')->getConfig('default_category_sort')) {
        ...
        for ($i = 0; $i < $result->getRows(); ++$i) {
            $category = new self((int) $result->getValue('category_id'), $clang_id);
            $categories[] = $category;
            $result->next();
        }
        return $categories;
        ...
        if ('missing' === $type) {
            $query = 'SELECT main.category_id FROM '. \rex::getTablePrefix() .'d2u_machinery_categories AS main '
                    ...
                    .'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_categories_lang AS default_lang '
                        .'ON main.category_id = default_lang.category_id AND default_lang.clang_id = :default_lang '
    ...
    public function getUrl($including_domain = false, $current_article_id = 0)
    {
        if ('' === $this->url) {
            ...
            $article_id = (int) $d2u_machinery->getConfig('article_id');

            // In case of used machines
            if (0 === $current_article_id) {
            ...
            if (Extension::isActive('used_machines') && ($current_article_id === (int) $d2u_machinery->getConfig('used_machine_article_id_rent') || $current_article_id === (int) $d2u_machinery->getConfig('used_machine_article_id_sale'))) {
                ...
                } elseif ('rent' === $this->offer_type) {
                    $article_id = (int) $d2u_machinery->getConfig('used_machine_article_id_rent');
                    // Set parameter key
     ...
     * @return bool true if childs exist, otherwise false
     */
     ...
     * @param bool $ignore_offlines Ignore offline used machines, default is false
     * @return bool true if used machines exist, otherwise false
     */
