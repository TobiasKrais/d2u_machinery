...
// Delete
	...
	if(count($reffering_machines) == 0) {
		foreach(rex_clang::getAll() as $rex_clang) {
			if($certificate === FALSE) {
				$certificate = new Certificate($certificate_id, $rex_clang->getId());
				// If object is not found in language, set certificate_id anyway to be able to delete
	...
	<form action="<?php print rex_url::currentBackendPage(); ?>" method="post">
		<div class="panel panel-edit">
			...
			<div class="panel-body">
						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() != rex_config::get("d2u_machinery", "default_lang")) {
									...
									d2u_addon_backend_helper::form_select('d2u_machinery_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, array($certificate->translation_needs_update), 1, FALSE, $readonly_lang);
								}
								else {
									print '<input type="hidden" name="form[lang]['. $rex_clang->getId() .'][translation_needs_update]" value="">';
								}
								
								d2u_addon_backend_helper::form_input('d2u_machinery_name', "form[lang][". $rex_clang->getId() ."][name]", $certificate->name, $required, $readonly_lang, "text");
								d2u_addon_backend_helper::form_textarea('d2u_machinery_certificates_description', "form[lang][". $rex_clang->getId() ."][description]", $certificate->description, 10, FALSE, $readonly_lang, TRUE)
							?>
						</div>
					</fieldset>
				<?php
				...
				?>
				<fieldset>
					<legend><?php echo rex_i18n::msg('d2u_machinery_certificates_data_all_lang'); ?></legend>
					<div class="panel-body-wrapper slide">
						<?php
