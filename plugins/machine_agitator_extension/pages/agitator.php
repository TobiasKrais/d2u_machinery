				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$agitator = new Agitator($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
							$readonly = TRUE;
...
if ($func === '') {
	$query = 'SELECT agitators.agitator_id, name '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_agitators_lang AS lang '
			. 'ON agitators.agitator_id = lang.agitator_id AND lang.clang_id = '. intval(rex_config::get("d2u_helper", "default_lang")) .' '
		. 'ORDER BY name ASC';
