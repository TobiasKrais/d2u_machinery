						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() !== intval(rex_config::get("d2u_helper", "default_lang"))) {
									...
									d2u_addon_backend_helper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$agitator_type->translation_needs_update], 1, FALSE, $readonly_lang);
								}
								else {
							...
							<div id="details_clang_<?php print $rex_clang->getId(); ?>">
								<?php
									d2u_addon_backend_helper::form_input('d2u_helper_name', "form[lang][". $rex_clang->getId() ."][name]", $agitator_type->name, $required, $readonly_lang, "text");
								?>
							</div>
						</div>
					</fieldset>
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$agitator_type = new AgitatorType($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
							$readonly = TRUE;
...
if ($func === '') {
	$query = 'SELECT agitator_types.agitator_type_id, name '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_agitator_types_lang AS lang '
			. 'ON agitator_types.agitator_type_id = lang.agitator_type_id AND lang.clang_id = '. intval(rex_config::get("d2u_helper", "default_lang")) .' '
		. 'ORDER BY name ASC';
