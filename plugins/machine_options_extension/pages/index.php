...
if (filter_input(INPUT_POST, "btn_save") == 1 || filter_input(INPUT_POST, "btn_apply") == 1) {
	...
	foreach(rex_clang::getAll() as $rex_clang) {
		if($option === FALSE) {
			...
			if(\rex_addon::get("d2u_videos")->isAvailable() && isset($form['video_id']) && $form['video_id'] > 0) {
				$option->video = new Video($form['video_id'], rex_config::get("d2u_helper", "default_lang"));
			}
			else {
				$option->video = FALSE;
			}
	...
	if(filter_input(INPUT_POST, "btn_apply") == 1 && $option !== FALSE) {
...
// Delete
	...
	$option = new Option($option_id, rex_config::get("d2u_helper", "default_lang"));
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$option = new Option($entry_id, rex_config::get("d2u_helper", "default_lang"));
							$readonly = TRUE;
...
if ($func == '') {
	$query = 'SELECT options.option_id, name, category_ids, priority '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_options_lang AS lang '
			. 'ON options.option_id = lang.option_id AND lang.clang_id = '. rex_config::get("d2u_helper", "default_lang") .' '
