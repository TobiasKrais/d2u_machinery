...
// Delete
	...
	$service_option = new ServiceOption($service_option_id, rex_config::get("d2u_helper", "default_lang"));
...
// Change online status of machine
	...
	$service_option = new ServiceOption($entry_id, rex_config::get("d2u_helper", "default_lang"));
...
?>
	<form action="<?php print rex_url::currentBackendPage(); ?>" method="post">
		<div class="panel panel-edit">
			<header class="panel-heading"><div class="panel-title"><?php print rex_i18n::msg('d2u_machinery_service_options'); ?></div></header>
			<div class="panel-body">
				<input type="hidden" name="form[service_option_id]" value="<?php echo $entry_id; ?>">
				<?php
					foreach(rex_clang::getAll() as $rex_clang) {
						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() != rex_config::get("d2u_helper", "default_lang")) {
									...
									d2u_addon_backend_helper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, array($service_option->translation_needs_update), 1, FALSE, $readonly_lang);
								}
								else {
...
if ($func == '') {
	$query = 'SELECT service_options.service_option_id, name, online_status '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_service_options_lang AS lang '
			. 'ON service_options.service_option_id = lang.service_option_id AND lang.clang_id = '. rex_config::get("d2u_helper", "default_lang") .' '
		. 'ORDER BY name ASC';
