...
class Profile implements \D2U_Helper\ITranslationHelper {
	...
	public static function getAll($clang_id) {
		...
		for($i = 0; $i < $result->getRows(); $i++) {
			$profiles[] = new Profile((int) $result->getValue("profile_id"), $clang_id);
			$result->next();
		}
		return $profiles;
	}
	...
	public static function getTranslationHelperObjects($clang_id, $type) {
		...
		$objects = [];
		for($i = 0; $i < $result->getRows(); $i++) {
			$objects[] = new Profile((int) $result->getValue("profile_id"), $clang_id);
			$result->next();
		}
	...
	public function save() {
		...
		if($this->profile_id === 0 || $pre_save_profile !== $this) {
			...
			$result->setQuery($query);
			if($this->profile_id === 0) {
				$this->profile_id = intval($result->getLastId());
