...
class Procedure implements \D2U_Helper\ITranslationHelper {
	...
	public static function getAll($clang_id) {
		...
		for($i = 0; $i < $result->getRows(); $i++) {
			$procedures[] = new Procedure((int) $result->getValue("procedure_id"), $clang_id);
			$result->next();
		}
		return $procedures;
	}
	...
	public static function getTranslationHelperObjects($clang_id, $type) {
		...
		$objects = [];
		for($i = 0; $i < $result->getRows(); $i++) {
			$objects[] = new Procedure((int) $result->getValue("procedure_id"), $clang_id);
			$result->next();
		}
	...
	public function save() {
		...
		if($this->procedure_id === 0 || $pre_save_procedure !== $this) {
			...
			$result->setQuery($query);
			if($this->procedure_id === 0) {
				$this->procedure_id = intval($result->getLastId());
