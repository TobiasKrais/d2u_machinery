...
if (intval(filter_input(INPUT_POST, "btn_save")) === 1 || intval(filter_input(INPUT_POST, "btn_apply")) === 1) {
	...
	if(intval(filter_input(INPUT_POST, "btn_apply", FILTER_VALIDATE_INT)) === 1 &&$automation !== false) {
...
// Delete
	...
	$automation = new Automation($automation_id, intval(rex_config::get("d2u_helper", "default_lang")));
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$automation = new Automation($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
							$readonly = true;
							if(\rex::getUser() instanceof rex_user && (\rex::getUser()->isAdmin() || \rex::getUser()->hasPerm('d2u_machinery[edit_data]'))) {
								$readonly = false;
							}
							d2u_addon_backend_helper::form_input('d2u_helper_name', "form[internal_name]", $automation->internal_name, true, $readonly, "text");
						?>
					</div>
				</fieldset>
				<?php
						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() !== intval(rex_config::get("d2u_helper", "default_lang"))) {
									...
									d2u_addon_backend_helper::form_select('d2u_helper_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, [$automation->translation_needs_update], 1, false, $readonly_lang);
								}
								else {
							...
							<div id="details_clang_<?php print $rex_clang->getId(); ?>">
								<?php
									d2u_addon_backend_helper::form_input('d2u_helper_name', "form[lang][". $rex_clang->getId() ."][name]", $automation->name, $required, $readonly_lang, "text");
								?>
							</div>
						</div>
					</fieldset>
...
if ($func === '') {
	$query = 'SELECT automations.automation_id, internal_name '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_steel_automation_lang AS lang '
			. 'ON automations.automation_id = lang.automation_id AND lang.clang_id = '. intval(rex_config::get("d2u_helper", "default_lang")) .' '
		. 'ORDER BY name ASC';
