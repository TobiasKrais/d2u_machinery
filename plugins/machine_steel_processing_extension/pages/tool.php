...
if (intval(filter_input(INPUT_POST, "btn_save")) === 1 || intval(filter_input(INPUT_POST, "btn_apply")) === 1) {
	...
	if(intval(filter_input(INPUT_POST, "btn_apply", FILTER_VALIDATE_INT)) === 1 &&$tool !== FALSE) {
...
// Delete
	...
	$tool = new Tool($tool_id, intval(rex_config::get("d2u_helper", "default_lang")));
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$tool = new Tool($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
							$readonly = TRUE;
...
if ($func === '') {
	$query = 'SELECT tools.tool_id, internal_name '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_steel_tool_lang AS lang '
			. 'ON tools.tool_id = lang.tool_id AND lang.clang_id = '. intval(rex_config::get("d2u_helper", "default_lang")) .' '
		. 'ORDER BY name ASC';
