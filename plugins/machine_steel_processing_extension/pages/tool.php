...
// Delete
	...
	$tool = new Tool($tool_id, rex_config::get("d2u_machinery", "default_lang"));
	
	// Check if object is used
	$reffering_machines = $tool->getRefferingMachines();
	...
	if(count($reffering_machines) == 0) {
		foreach(rex_clang::getAll() as $rex_clang) {
			if($tool === FALSE) {
				$tool = new Tool($tool_id, $rex_clang->getId());
				// If object is not found in language, set tool_id anyway to be able to delete
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$tool = new Tool($entry_id, rex_config::get("d2u_machinery", "default_lang"));
							$readonly = TRUE;
							if(rex::getUser()->isAdmin() || rex::getUser()->hasPerm('d2u_machinery[edit_tech_data]')) {
								$readonly = FALSE;
							}
							d2u_addon_backend_helper::form_input('d2u_machinery_name', "form[internal_name]", $tool->internal_name, TRUE, $readonly, "text");
						?>
					</div>
				</fieldset>
				<?php
						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() != rex_config::get("d2u_machinery", "default_lang")) {
									...
									d2u_addon_backend_helper::form_select('d2u_machinery_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, array($tool->translation_needs_update), 1, FALSE, $readonly_lang);
								}
								else {
									print '<input type="hidden" name="form[lang]['. $rex_clang->getId() .'][translation_needs_update]" value="">';
								}
								
								d2u_addon_backend_helper::form_input('d2u_machinery_name', "form[lang][". $rex_clang->getId() ."][name]", $tool->name, $required, $readonly_lang, "text");
							?>
						</div>
					</fieldset>
				<?php
...
if ($func == '') {
	$query = 'SELECT tools.tool_id, internal_name '
		...
		. 'LEFT JOIN '. rex::getTablePrefix() .'d2u_machinery_steel_tool_lang AS lang '
			. 'ON tools.tool_id = lang.tool_id AND lang.clang_id = '. rex_config::get("d2u_machinery", "default_lang") .' '
		. 'ORDER BY name ASC';
