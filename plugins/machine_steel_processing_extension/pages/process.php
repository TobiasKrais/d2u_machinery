...
// Delete
	...
	$process = new Process($process_id, rex_config::get("d2u_machinery", "default_lang"));
	...
	if(count($reffering_machines) == 0) {
		foreach(rex_clang::getAll() as $rex_clang) {
			if($process === FALSE) {
				$process = new Process($process_id, $rex_clang->getId());
				// If object is not found in language, set process_id anyway to be able to delete
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$process = new Process($entry_id, rex_config::get("d2u_machinery", "default_lang"));
							$readonly = TRUE;
						...
						<div class="panel-body-wrapper slide">
							<?php
								if($rex_clang->getId() != rex_config::get("d2u_machinery", "default_lang")) {
									...
									d2u_addon_backend_helper::form_select('d2u_machinery_translation', 'form[lang]['. $rex_clang->getId() .'][translation_needs_update]', $options_translations, array($process->translation_needs_update), 1, FALSE, $readonly_lang);
								}
								else {
...
if ($func == '') {
	$query = 'SELECT processs.process_id, internal_name '
		...
		. 'LEFT JOIN '. rex::getTablePrefix() .'d2u_machinery_steel_process_lang AS lang '
			. 'ON processs.process_id = lang.process_id AND lang.clang_id = '. rex_config::get("d2u_machinery", "default_lang") .' '
		. 'ORDER BY name ASC';
