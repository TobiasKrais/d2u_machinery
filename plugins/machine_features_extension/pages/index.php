...
if (intval(filter_input(INPUT_POST, "btn_save")) === 1 || intval(filter_input(INPUT_POST, "btn_apply")) === 1) {
	...
	foreach(rex_clang::getAll() as $rex_clang) {
		if($feature === FALSE) {
			...
			$feature->category_ids = is_array($form['category_ids']) ? $form['category_ids'] : [];
			...
			if(\rex_addon::get("d2u_videos")->isAvailable() && isset($form['video_id']) && $form['video_id'] > 0) {
				$feature->video = new Video($form['video_id'], intval(rex_config::get("d2u_helper", "default_lang")));
			}
			else {
				$feature->video = FALSE;
			}
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							$feature = new Feature($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
							$readonly = TRUE;
...
if ($func === '') {
	$query = 'SELECT features.feature_id, name, category_ids, priority '
		...
		. 'LEFT JOIN '. \rex::getTablePrefix() .'d2u_machinery_features_lang AS lang '
			. 'ON features.feature_id = lang.feature_id AND lang.clang_id = '. intval(rex_config::get("d2u_helper", "default_lang")) .' '
