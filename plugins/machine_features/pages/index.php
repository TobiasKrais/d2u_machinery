...
if (filter_input(INPUT_POST, "btn_save") == 1 || filter_input(INPUT_POST, "btn_apply") == 1) {
	...
	foreach(rex_clang::getAll() as $rex_clang) {
		if($feature === FALSE) {
			...
			$feature->category_ids = preg_grep('/^\s*$/s', explode("|", $form['videomanager_ids']), PREG_GREP_INVERT);
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							...
							d2u_addon_backend_helper::form_mediafield('d2u_machinery_features_pic', 'form[pic]', $feature->pic, $readonly);
							
							$options = array();
							foreach(Category::getAll(rex_config::get("d2u_machinery", "default_lang")) as $category) {
								$options[$category->feature_id] = $category->name;
							}
